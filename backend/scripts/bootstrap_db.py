...
def main() -> None:
    ...
    # exits non-zero, so a branched revision chain or a silent no-op upgrade fails
    # the `migrate` step loudly instead of letting api/worker boot on a stale schema.
    asyncio.run(assert_db_at_head(dsn))
