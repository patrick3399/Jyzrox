...
class FanboxSourcePlugin(SourcePlugin):
    ...
    meta = PluginMeta(
        ...
        credential_schema=[FieldDef(name="fanboxsessid", label="FANBOXSESSID Cookie", field_type="password", required=False)],
        supported_sites=[SiteInfo(domain="fanbox.cc", name="Pixiv Fanbox", source_id="fanbox", category="art", has_tags=True)],
        concurrency=1,
    ...
    async def discover_posts(
        ...
        headers = {"Accept": "application/json, text/plain, */*", "Referer": "https://www.fanbox.cc/", "User-Agent": "Mozilla/5.0"}
        works: list[NewWork] = []
        latest_id: str | None = None
        async with httpx.AsyncClient(headers=headers, cookies=_cookies(credentials), follow_redirects=True, timeout=45) as client:
            ...
            for page_url in pages:
                ...
                for item in posts:
                    ...
                    if policy.includes_fee(fee):
                        posted_at = None
                        try:
                            posted_at = datetime.fromisoformat(str(item.get("publishedDatetime", "")).replace("Z", "+00:00"))
                        except ValueError:
                            pass
                        works.append(NewWork(
                            url=f"https://www.fanbox.cc/@{creator_id}/posts/{post_id}",
                            title=str(item.get("title") or ""), source_id=post_id,
                            thumbnail_url=item.get("coverImageUrl"), posted_at=posted_at,
                        ))
                if reached_boundary:
                    break
    ...
    def credential_flows(self) -> list[CredentialFlow]:
        return [CredentialFlow(flow_type="fields", fields=[FieldDef(name="fanboxsessid", label="FANBOXSESSID Cookie", field_type="password", required=False, placeholder="Paste the FANBOXSESSID cookie value")])]
    ...
    async def download(
        self, url: str, dest_dir: Path, credentials: dict | str | None = None,
        ...
        match = _POST_RE.search(url)
        if not match:
            return DownloadResult(status="failed", downloaded=0, total=0, error="Fanbox requires an individual post URL")
        try:
            policy = fanbox_policy_from_options(options)
        except ValueError as exc:
            return DownloadResult(status="failed", downloaded=0, total=0, error=f"Invalid Fanbox download policy: {exc}")
        ...
        async with httpx.AsyncClient(headers=headers, cookies=_cookies(credentials), follow_redirects=True, timeout=45) as client:
            ...
            failed: list[int] = []
            metadata = {
                "category": "fanbox", "id": post_id, "gallery_id": post_id,
                "title": post.get("title") or f"fanbox_{post_id}",
                "uploader": post.get("creatorId") or "", "date": post.get("publishedDatetime"),
                "description": post.get("body", {}).get("text", "") if isinstance(post.get("body"), dict) else "",
                "fee_required": fee, "creator_id": post.get("creatorId"), "post_type": post.get("type"),
            }
            for index, media_url in enumerate(urls, start=1):
                if cancel_check and await cancel_check():
                    return DownloadResult(status="cancelled", downloaded=downloaded, total=len(urls), failed_pages=failed)
                ...
                try:
                    ...
                    path = dest_dir / f"{post_id}_p{index:04d}{_extension(media_url, media.headers.get('content-type'))}"
                    path.write_bytes(media.content)
                    path.with_name(path.name + ".json").write_text(json.dumps(metadata, ensure_ascii=False), encoding="utf-8")
                    downloaded += 1
                    if on_file:
                    ...
                    failed.append(index)
        return DownloadResult(status="partial" if failed and downloaded else "done", downloaded=downloaded, total=len(urls), failed_pages=failed)
