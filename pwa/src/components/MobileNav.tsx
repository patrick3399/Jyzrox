...
export function MobileNav({ open, onClose, downloadStats }: MobileNavProps) {
  const pathname = usePathname()

  // Close drawer on route change — stays in outer shell
  useEffect(() => {
    onClose()
  }, [pathname, onClose])

  // Prevent body scroll when drawer is open — stays in outer shell
  useEffect(() => {
    if (open) {
  ...
  return (
    <>
      {/* Backdrop — always rendered for CSS transition */}
      <div
      ...
      {/* Drawer shell — always rendered for slide transition */}
      <aside
      ...
      >
        {open && <MobileNavContent onClose={onClose} downloadStats={downloadStats} />}
      </aside>
    </>
  )
}

// All heavy hooks live here — only executed when drawer is open
