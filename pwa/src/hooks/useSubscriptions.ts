...
export function useSubscriptions(params: { source?: string; enabled?: boolean; limit?: number; offset?: number } = {}) {
  const key = ['subscriptions', JSON.stringify(params)]
  return useSWR(key, () => api.subscriptions.list(params))
}

export function useCreateSubscription() {
  return useSWRMutation('subscriptions', (_key: string, { arg }: { arg: { url: string; name?: string; cron_expr?: string; auto_download?: boolean } }) =>
    api.subscriptions.create(arg)
  )
}

export function useUpdateSubscription() {
  return useSWRMutation('subscriptions', (_key: string, { arg }: { arg: { id: number; data: { name?: string; enabled?: boolean; auto_download?: boolean; cron_expr?: string } } }) =>
